...
def ordenar_columnas_aire(df: pd.DataFrame) -> list:
    ...
    for cont, est in pares_ord:
        ...
        col_cant_candidates = [c for c in df.columns if c.endswith(f"_{cont}_{est}") and c.startswith("CANTIDAD_")]
...
def extraer_estaciones_aire(df: pd.DataFrame, orden_cat: dict, suficiencia: float) -> dict:
    ...
    resultado = {}
    for est in sorted(estaciones):
        cols_cat  = [c for c in df.columns if _PAT_AIRE.match(c) and _PAT_AIRE.match(c).group(2) == est]
        cols_cant = [c for c in df.columns if _PAT_CANT.match(c) and _PAT_CANT.match(c).group(3) == est]
        ...
        for cont, est2 in sorted(pares, key=lambda x: x[0]):
            ...
            col_cant = [c for c in df_est.columns if c.endswith(f"_{cont}_{est2}") and c.startswith("CANTIDAD_")]
    ...
    with pd.ExcelWriter(archivo, engine="openpyxl") as writer:
        for nombre_hoja, df in diccionario_dfs.items():
            ...
            if tipo == "DIARIO":
                ...
                df_export[nombre_indice] = pd.to_datetime(df_export[nombre_indice]).dt.strftime("%Y-%m-%d")
